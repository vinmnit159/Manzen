...
import {
  X, FileText, FlaskConical, Download, ExternalLink, Upload,
  CheckCircle2, Clock, Edit3, AlertCircle, Tag, Loader2, BookOpen,
} from "lucide-react";
...
export function PolicyDetailPanel({
  ...
  const relatedTests = allTests
    .filter((t: any) => t.category === "Policy" || t.name.toLowerCase().includes(policyWord))
    .slice(0, 6);
      ...
      <div className="relative z-50 w-full max-w-2xl bg-white shadow-2xl flex flex-col h-full overflow-hidden">
        ...
        <div className="flex items-start justify-between px-5 py-4 border-b border-gray-200 bg-white sticky top-0">
          <div className="flex-1 min-w-0 pr-3">
            ...
            <h2 className="text-lg font-semibold text-gray-900 leading-snug">{policy.name}</h2>
            {policy.approvedBy && (
              <p className="text-xs text-gray-500 mt-1">Approved by <span className="font-medium text-gray-700">{policy.approvedBy}</span></p>
            )}
          </div>
        ...
        <div className="px-5 py-3 border-b border-gray-100 bg-gray-50">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-gray-900">{relatedControls.length}</p>
              <p className="text-xs text-gray-500 mt-0.5">Related controls</p>
            </div>
            <div className="text-center">
              <p className={`text-2xl font-bold ${policy.documentUrl ? "text-green-600" : "text-gray-400"}`}>
                {policy.documentUrl ? "✓" : "—"}
              </p>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          <Tabs defaultValue="overview" className="space-y-4">
            <TabsList className="rounded-xl bg-slate-100 p-1 h-auto">
              ...
              <TabsTrigger value="controls">Controls ({relatedControls.length})</TabsTrigger>
              <TabsTrigger value="document">Document</TabsTrigger>
            </TabsList>

            {/* Overview */}
            <TabsContent value="overview" className="space-y-4 mt-0">
              <Section title="Policy Details" icon={<FileText className="w-4 h-4 text-gray-500" />}>
                <dl className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    <dt className="text-xs font-medium text-gray-400 uppercase tracking-wide">Version</dt>
                    <dd className="mt-1 font-mono font-semibold text-gray-700">v{policy.version}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-gray-400 uppercase tracking-wide">Created</dt>
                    <dd className="mt-1 text-gray-700">{fmtDate(policy.createdAt)}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-gray-400 uppercase tracking-wide">Last Updated</dt>
                    <dd className="mt-1 text-gray-700">{fmtDate(policy.updatedAt)}</dd>
                  </div>
                  {policy.approvedBy && (
                    <div className="col-span-2">
                      <dt className="text-xs font-medium text-gray-400 uppercase tracking-wide">Approved By</dt>
                      <dd className="mt-1 text-gray-700">{policy.approvedBy}</dd>
                    </div>
                  )}
                  {policy.approvedAt && (
                    <div>
                      <dt className="text-xs font-medium text-gray-400 uppercase tracking-wide">Approved At</dt>
                      <dd className="mt-1 text-gray-700">{fmtDate(policy.approvedAt)}</dd>
                    </div>
                  )}
                </dl>
              </Section>
              ...
              {relatedTests.length > 0 && (
                <Section title={`Related Tests (${relatedTests.length})`} icon={<FlaskConical className="w-4 h-4 text-gray-500" />}>
                    ...
                    {relatedTests.slice(0, 4).map((t: any) => {
                      ...
                      return (
                        <li key={t.id} className="flex items-center justify-between p-2.5 rounded-xl border border-gray-100 bg-gray-50">
                          ...
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ml-2 ${badgeColors[t.status] ?? "bg-gray-100 text-gray-600"}`}>
            ...
            <TabsContent value="tests" className="space-y-3 mt-0">
              {relatedTests.length === 0 ? (
                ...
                return (
                  <div key={t.id} className={`rounded-xl border p-4 ${badgeColors[t.status] ?? "border-gray-200"}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-gray-900">{t.name}</p>
                        <p className="text-xs text-gray-500 mt-0.5">{t.type} · {t.category} · Due {fmtDate(t.dueDate)}</p>
                      </div>
                      <span className={`text-xs px-2.5 py-1 rounded-full font-medium flex-shrink-0 ${textColors[t.status] ?? "bg-gray-100 text-gray-600"}`}>
                        {t.status.replace(/_/g, " ")}
                      </span>
                    </div>
                  </div>
            ...
            <TabsContent value="controls" className="space-y-3 mt-0">
              {relatedControls.length === 0 ? (
                <div className="text-center py-12 text-gray-400">
                  ...
                  <p className="text-sm">No related controls found for this policy.</p>
                </div>
                ...
                return (
                  <div key={c.id} className="flex items-center justify-between p-3 rounded-xl border border-gray-100 bg-gray-50">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ml-2 ${sColors[c.status] ?? "bg-gray-100 text-gray-600"}`}>
            ...
            <TabsContent value="document" className="space-y-4 mt-0">
              {policy.documentUrl ? (
                <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5 space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">Attached document</p>
                    </div>
                  </div>
                ...
                >
                  ...
                  <p className="text-sm font-medium text-gray-700">Click to upload a policy document</p>
                  <p className="text-xs text-gray-400">PDF, Word, Excel — max 50 MB</p>
                </div>
              )}
              ...
              {uploadErr && (
                <p className="text-sm text-red-600 bg-red-50 rounded-lg px-3 py-2">{uploadErr}</p>
              )}
            </TabsContent>
          </Tabs>
        </div>
