...
import {
  CheckCircle2, ArrowRight, ShieldCheck, ListChecks, AlertTriangle,
  Link2, Target, Info, SkipForward,
} from "lucide-react";
import type { ActivationSummaryDto, OrgFrameworkDto } from "@/services/api/frameworks";
    ...
    <div className="flex items-start gap-4 p-4 rounded-xl border border-gray-100 bg-white hover:border-blue-200 hover:bg-blue-50/30 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          ...
          {optional && <Badge variant="outline" className="text-xs text-gray-400 border-gray-200">Optional</Badge>}
        </div>
...
export function ActivationSummaryPage() {
  ...
  return (
    <PageTemplate
      ...
      description={frameworkName + (frameworkVersion ? ` v${frameworkVersion}` : '')}
    >
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <Card className="border-green-200 bg-gradient-to-br from-green-50 to-white">
          <CardContent className="pt-6 pb-5">
            <div className="flex items-center gap-4">
              <div className={`w-14 h-14 rounded-2xl ${frameworkColor(slug)} flex items-center justify-center flex-shrink-0`}>
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold text-gray-900">
                    {summary?.isReactivation ? 'Framework Re-activated' : 'Framework Activated'}
                  </h2>
                </div>
                <p className="text-sm text-gray-600 mt-0.5">
                  <strong>{frameworkName}</strong> is now in scope for your organization.
                  {summary?.isReactivation && ' Your existing data, mappings, and history were preserved.'}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        {summary && (
          <div className="space-y-3">
            <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Setup Summary</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              />
              <StatCard
                value={`${summary.initialCoverageScore}%`}
            ...
            {summary.warnings.length > 0 && (
              <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-xs font-semibold text-amber-800 uppercase tracking-wide">Notes</p>
                </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Next Steps</h3>
            <Button
              variant="ghost"
              size="sm"
          ...
          <div className="space-y-2">
            <NextStepCard
              ...
              actionLabel="Review"
              onAction={() => navigate(`/compliance/frameworks/${slug}`, { state: { tab: 'requirements' } })}
            />
            <NextStepCard
              stepNum={2}
              icon={Link2}
              ...
              actionLabel="Review"
              onAction={() => navigate(`/compliance/frameworks/${slug}`, { state: { tab: 'controls' } })}
            />
            <NextStepCard
              stepNum={3}
              icon={Target}
              ...
              actionLabel="Review"
              onAction={() => navigate(`/compliance/frameworks/${slug}`, { state: { tab: 'policies' } })}
            />
            <NextStepCard
              stepNum={4}
              icon={AlertTriangle}
              ...
              onAction={() => navigate(`/compliance/frameworks/${slug}`, { state: { tab: 'gaps' } })}
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-2">
          <Button variant="outline" onClick={() => navigate('/compliance/frameworks')}>
            Back to Frameworks
          </Button>
