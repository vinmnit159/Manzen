...
import { computePercentage, computeTopicSummaries, getTopicLabel, getTakeawaysForMistakes, isRetakeRecommended } from '../lib/scoring';
...
export function SummaryScreen({ state, onComplete, onRetake }: SummaryScreenProps) {
  ...
  const topicSummaries = computeTopicSummaries(state.correctByTopic, state.mistakesByTopic);
  ...
  return (
    <div className="max-w-xl mx-auto space-y-5 py-4">
      {/* Mascot + Rating hero */}
      <div className={`rounded-2xl border-2 ${ratingCfg.border} ${ratingCfg.bg} p-5 text-center space-y-3`}>
        <div className="flex justify-center">
          <div className="w-14 h-14 rounded-2xl bg-white/80 flex items-center justify-center shadow-sm">
            <RatingIcon className={`w-7 h-7 ${ratingCfg.color}`} aria-hidden="true" />
          </div>
        </div>
        <div>
        ...
        <div className="flex items-start gap-2 bg-white/60 rounded-lg px-3 py-2 text-left">
          <Dog className="w-5 h-5 text-slate-500 flex-shrink-0 mt-0.5" aria-hidden="true" />
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-xl border border-gray-200 bg-white p-3 text-center">
          <p className="text-2xl font-bold font-mono text-gray-900">{state.badges.length}</p>
          <p className="text-xs text-gray-500">Badges</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-3 text-center">
          <p className="text-2xl font-bold font-mono text-gray-900">{state.bestStreak}</p>
          <p className="text-xs text-gray-500">Best Streak</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-3 text-center">
          <p className="text-2xl font-bold font-mono text-gray-900">{minutes}:{seconds.toString().padStart(2, '0')}</p>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 space-y-3">
        ...
        <div className="space-y-2">
          {ALL_MODULES.map(mod => {
            ...
            const modPct = modMax > 0 ? Math.round((modScore / modMax) * 100) : 0;
            return (
              <div key={mod.id} className="flex items-center gap-3">
                <span className="text-xs text-gray-500 w-28 truncate">{mod.shortTitle}</span>
                ...
                <span className="text-xs font-mono text-gray-600 w-10 text-right">{modPct}%</span>
              </div>
            );
          })}
        </div>
      ...
      {strengths.length > 0 && (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {strengths.map(s => (
              <span key={s.topic} className="px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-700 text-xs font-medium">
      ...
      {weaknesses.length > 0 && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {weaknesses.map(w => (
              <span key={w.topic} className="px-2.5 py-1 rounded-full bg-amber-100 text-amber-700 text-xs font-medium">
      ...
      {retake && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-red-900">Retake recommended</p>
            <p className="text-xs text-red-700 mt-0.5">
              Your score indicates some topics need more review. Consider retaking the training to strengthen your knowledge.
            </p>
          </div>
        </div>
      )}
