...
export function getPostgresPool(config?: PostgresRuntimeConfig) {
  if (!config) throw new Error('Postgres config is required to initialize the pool.');
...
export function readPostgresRuntimeConfig(): PostgresRuntimeConfig | null {
  ...
  return {
    connectionString,
    ssl: typeof process !== 'undefined' && process.env.PGSSLMODE === 'require' ? { rejectUnauthorized: false } : undefined,
    max: typeof process !== 'undefined' && process.env.PGPOOL_MAX ? Number(process.env.PGPOOL_MAX) : undefined,
  };
}
